...
FLAGS.add_argument('--model', type=str, default='output/baseline/cifar-20/selflabel/best_model518.pth.tar', help='Location where model is saved')
...
def main():
    ...
    if config['setup'] in ['simclr', 'moco']:
        ...
        if args.visualize_prototypes:
            ...
            visualize_indices(prototype_indices, dataset, clustering_stats['hungarian_match'], output_dir=os.path.join('../results/figures/prototype', config['dataset_name']))
    else:
        raise NotImplementedError
...
def visualize_indices(indices, dataset, hungarian_match, output_dir=None):
    ...
    for idx in indices:
        ...
        if output_dir:
            img.save(os.path.join(output_dir, 'label_{}.png'.format(label_i)))
            label_i = label_i + 1
        # plt.figure()
        # plt.axis('off')
        # plt.imshow(img)
